...
struct Sector {
	Sector(const WADSector& from, WADLoader *wad); int16_t FloorHeight, CeilingHeight;
	const Texture *floortexture, *ceilingtexture; uint16_t Lightlevel, Type, Tag;
};
struct Sidedef {
	Sidedef(const WADSidedef &from, const std::vector<Sector> &sectors, WADLoader *wad);
	int16_t XOffset, YOffset; const Texture *uppertexture, *middletexture, *lowertexture; const Sector *pSector;
};
struct Linedef {
	Linedef(const WADLinedef &from, const std::vector<Sidedef> & sidedefs, const std::vector<Vertex> &vertices);
