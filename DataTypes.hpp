...
struct Sidedef {
	int16_t dx, dy; const Texture *uppertexture, *middletexture, *lowertexture; const Sector *sector;
};
struct Linedef {
	Vertex start, end; uint16_t flags, type, sectorTag; const Sidedef *rSidedef, *lSidedef;
};
struct Seg {
	Vertex start, end; float slopeAngle; const Linedef *linedef; uint16_t direction; float offset; const Sector *rSector, *lSector; // Direction: 0 same as linedef, 1 opposite of linedef. Offset: distance along linedef to start of seg.
};
...
public:
	...
	Thing* GetThingByID(int id) { for (auto& t : things) if (t.type == id) return &t; return nullptr; }
protected:
